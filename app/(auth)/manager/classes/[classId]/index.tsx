...
export default function EditClass() {
  ...
  const [data, setData] = useState(selectedClass);
  ...
  return (
    <PageContainer as={ScrollView}>
      <View style={stylesItem.container}>
        ...
        />
        <Button style={stylesItem.button} size="large" onPress={() => dataConfirmationHandler(data!, router, updateClass)}>
...
function DateTimeInput({ startingDate, setStartingDate }: DateTimeInputProps) {
  ...
  const [date, setDate] = useState(startingDate.toLocaleDateString("en-GB"));
  const [time, setTime] = useState(startingDate.toLocaleTimeString([], {
    hour: "2-digit",
    minute: "2-digit",
    hour12: false,
  }));
...
function dataConfirmationHandler(data: GestorClassCreateProps, router: Router, updateClass: (classId: string, classData: Partial<Omit<GestorClassProps, "id">>) => Promise<boolean>) {
